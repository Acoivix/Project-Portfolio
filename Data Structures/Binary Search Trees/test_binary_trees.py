...
def test_bst_remove(build_the_tree):
    """Unit Test for Problem 4: creating a remove method for your BST class."""
    tree = build_the_tree
    
    # Raise Value Error for trying to remove nonexistant node
    with pytest.raises(ValueError):
        tree.remove(10), "Did not raise a value error for trying to remove a nonexistant node"

    # Raise Value Error for no root
    new_tree = binary_trees.BST()
    with pytest.raises(ValueError):
        new_tree.remove(1), "Did not raise value error for tree being empty"
    
    # Case 1 (Remove a Leaf)
    tree.remove(2)
    assert ",2 " not in tree.__str__()

    # Case 2 (Remove a node with one child)
    tree.remove(9)
    assert ",9 " not in tree.__str__()

    # # Case 3 (Remove a node with two children)
    tree.remove(6)
    assert ",6 " not in tree.__str__()
